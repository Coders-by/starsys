...
export default function LifeWalk() {
  ...
  const coLivingEvents: Record<string, {
    ...
    balcony: {
      title: "🌌 11:20 PM ㆍ 阳台冷风与科技巨楼",
      desc: "站在合租房的阳台上，南山午夜的微风带着海湾微咸的湿热。远处腾讯大厦和科兴科学园的高光在夜云里发红，那是万千服务器的数据流在狂吠。",
      choices: [
        {
          label: "凝视黑客科技丛林",
          id: "gaze",
          result: "【虚无的数字化映射】：高空万点荧光，人类的悲欢全被规整为万亿比特。你突然明白，自己不是高耸王台里的观测者，而只是一个用体温取暖、踩在泥土水槽里的具体呼吸个体。",
          effect: () => { setHumanValue(p => p + 12); }
        },
        {
          label: "伸手收晾干的衬衫",
          id: "cloth",
          result: "【具体烟火的厚重感】：去收取在夜色里晾干的旧棉质衬衫。指尖触到干净洗衣粉的清香，以及衣服随风飘散的热度。琐碎和温软，才是托起一个天才灵魂最终免于悬空坠亡的实体防护网。",
          effect: () => { setHumanValue(p => p + 15); }
        }
      ]
    },
    kitchen: {
      title: "🍜 11:40 PM ㆍ 铁锅、鸡蛋与下碗面",
      desc: "肚子发出落寞的响声。深夜的厨房被黄晕吊灯笼罩。水龙头滴答滴答。要不要为自己做点什么充饥？",
      choices: [
        {
          label: "下一包寿桃面，加单面溏心蛋",
          id: "ramen",
          result: "【具体的温度治愈】：清脆的敲蛋声在厨壁碰着瓷响。切一小撮小葱末，滚烫的寿桃面汤浮起翠绿。金黄蛋黄在口中缓缓流出，那一刻被胃部的暖色深深包住，王阳明的知行合一，就实现在这一热气腾腾的碗羹汤里。",
          effect: () => { setHumanValue(p => p + 18); }
        },
        {
          label: "把多日水槽里的油盘仔细擦净",
          id: "cleaning",
          result: "【实体秩序重建】：拿海绵，挤洗洁精，刷擦水盘。不锈钢的明亮光晕被细密泡沫寸寸摩擦出来。水龙头流过舒缓的水声，生活在指缝中找到了实实在在的工作载荷。",
          effect: () => { setHumanValue(p => p + 8); }
        }
      ]
    },
    sofa: {
      title: "🛋️ 12:00 AM ㆍ 舍友漫谈与昏黄白炽灯",
      desc: "合租房的客厅极小，老式的灯带闪烁。累倒在沙发一侧，舍友正吃着烧烤，抬头问了你一眼：『千岑，今天实车标定跑得怎么样？手势算法还误触不？』",
      choices: [
        {
          label: "胡吹海侃/聊今晚买菜成本",
          id: "chat",
          result: "【热诚无拘的人性碰流】：你们从今晚超市的黄瓜折扣，一路扯到硬件工厂CNC铝合金切削的排版废料。你卸下了高傲博学的主讲架子，在最世俗最琐屑的对骂中，感到血管里的冰块被热气腾腾地熨化了。",
          effect: () => { setHumanValue(p => p + 16); }
        },
        {
          label: "在沙发里沉沉打呼睡去",
          id: "sleep",
          result: "【无懈可击的安全区】：不解释，不分析。直接缩进松软的灯芯绒靠垫。没有算法，没有关系警报，唯有空气中残留的一丝烤串孜然香气，陪伴你滑入彻底不求解答的睡眠里。",
          effect: () => { setHumanValue(p => p + 10); }
        }
      ]
    }
      ...
      <div className="bg-gradient-to-r from-stone-900 via-stone-950 to-stone-900 border border-stone-850 p-4 rounded-2xl shadow-xl flex justify-between items-center relative overflow-hidden">
        ...
        <div className="space-y-1 relative z-10">
          <div className="flex items-center gap-1.5 text-amber-300 font-extrabold text-xs">
            <span>🦊 🌸【人格第四维度：烟火人间 ㆍ 凡间漫游】</span>
          </div>
          <p className="text-[10px] text-stone-400 max-w-[240px] leading-relaxed text-justify">
            在前三个维度里，我们反思、寻找、调试，一直向内寻找宇宙答案。但真正的千岑，是在吃一碗溏心荷包蛋、听一曲旧歌、撮合一个落地的百万元硬核项目中，在深圳南山夜风下活出温度的具体生命。
          </p>
        </div>
          ...
          {!currentSong ? (
            ...
            <div className="bg-stone-950 p-4 rounded-xl border border-stone-850 space-y-4 animate-[fadeIn_0.3s_ease_1]">
              ...
              {singProgress < 100 ? (
                ...
                <div className="p-3.5 bg-rose-500/5 border border-rose-500/20 text-stone-300 text-[10.5px] leading-relaxed text-justify font-mono rounded-xl space-y-1.5 animate-pulse">
                  <span className="text-[10px] font-bold text-rose-400 block">🗝️ 捕获隐藏情感痕印：</span>
                  <p className="italic text-stone-200">
                    “在这句歌词里，我找到了当年在深圳南山徒步痛哭时丢掉的半块石英。我曾判决自己无法给予他人稳定长久的幸福，因此用算法自我流放。那一卷纸抄本夹着的青海祁连山干花，其实就是当年命运给予我最大的美善款待。”
                  </p>
                </div>
              )}
            </div>

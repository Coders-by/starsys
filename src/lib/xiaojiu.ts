 ...
 *
 * 密度对标 achievements.ts：一句具体场景开场，一个动作贯穿，不堆物象，
 * 不撒自造词（软隔离垫/容错美学/手刹/观察者装甲/筏造者 一律不用）。
 */
export function pickFallback(message: string): string {
  ...
  if (
    ...
    return '千岑以前爱在脑子里高谈阔论，直到他在《卡拉马佐夫兄弟》里读到老陀那句——要爱具体的普通人，胜过爱抽象的人类。读完那阵他做了件具体的小事：把深夜带车路试的细节写成带教文档，交给快崩溃的新人。你现在问的这一卷，就是他那时候的笔记。要小九替你翻哪一页？';
  }
  ...
  if (
    ...
    return '《镜像回声》是他写给自己的——那阵子青年虚无、夜里冷得空。他没去贩卖职场焦虑，只是把他调自动驾驶冗余系统时练出来的"先接住、再降级"，挪去接住自己。这玩意儿不是鸡汤。你问了，小九就陪你读一段。';
  }
  ...
  if (
    ...
    return '黑河零下 40 度，轮毂在失控边缘抱死的那一下，他蹲在车底想的是——系统出问题可以降级保命，那人焦虑的时候是不是也该拉一下手刹，别死扛。这事儿他后来真写进了带教文档。你想看哪一段路试的细节？';
  }

  return '服务器刚好刮过一阵风，小九和主人的天线歪了一丁点——有备用白茶。你想去档案里哪一页看看？小九陪你。';
}

/** 当 Gemini 返回空 reply 时使用的最小占位回复。 */
export const XIAOJIU_EMPTY_REPLY = '风太大了，小九没听清。🍵 你再说一遍？';
